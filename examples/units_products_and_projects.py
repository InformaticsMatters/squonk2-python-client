...
# Create the unit.
as_rv: AsApiRv = as_api.create_unit(
    as_token,
    org_id=org_id,
    billing_day=8,
...
dm_rv: DmApiRv = dm_api.create_project(
    dm_token,
...
dm_rv = dm_api.delete_project(dm_token, project_id=project_id)
if as_rv.success:
...
as_rv = as_api.delete_product(as_token, product_id=dt_product_id)
if as_rv.success:
...
as_rv = as_api.delete_product(as_token, product_id=storage_product_id)
if as_rv.success:
...
as_rv = as_api.delete_unit(as_token, unit_id=unit_id)
if as_rv.success:
    print("Unit deleted")
else:
